...
public class CSRFFilter {
    ...
    public static func filter(_ request: HTTPRequest) -> Bool {
        ...
        if session.state == "new" && request.method == .post {
    ...
    public static func setCookie(_ response: HTTPResponse) {
        ...
        var domain = ""
        if !CSSessionConfig.cookieDomain.isEmpty {
            domain = CSSessionConfig.cookieDomain
        }
        if let t = session.data["csrf"] {
            response.addCookie(HTTPCookie(
                ...
                value: "\(t)",
                domain: domain,
                expires: .relativeSeconds(CSSessionConfig.idle),
                path: CSSessionConfig.cookiePath,
                secure: CSSessionConfig.cookieSecure,
                httpOnly: CSSessionConfig.cookieHTTPOnly,
                sameSite: CSSessionConfig.cookieSameSite
            ))
        }
    }
